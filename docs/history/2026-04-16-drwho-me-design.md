...
> **STATUS: SUPERSEDED 2026-04-29** by [`2026-04-29-evidence-pack-pivot.md`](./2026-04-29-evidence-pack-pivot.md).
> The original "fast minimal monetised tools site + AdSense + paid MCP" framing has been retired in favour of an Evidence Pack-led product: paid signed PDF artifacts ($29/pack) + monitoring subscriptions ($19/$79/$249). Tool surface is tiered (core dossier checks central, IP/UA as "domain context"); 7 pure dev tools (base64, JWT, URL encode, UUID, JSON formatter) are deleted.
> Read for historical context only. Do not implement.
